...
let activeInlineWorkers = 0;
let inlineGlobalBaseline: Map<PropertyKey, PropertyDescriptor> | undefined;

function captureGlobalBaseline(): Map<PropertyKey, PropertyDescriptor> {
	const baseline = new Map<PropertyKey, PropertyDescriptor>();
	for (const key of Reflect.ownKeys(globalThis)) {
		const descriptor = Object.getOwnPropertyDescriptor(globalThis, key);
		if (descriptor) baseline.set(key, descriptor);
	}
	return baseline;
}

function retainInlineGlobalBaseline(): void {
	if (activeInlineWorkers === 0) inlineGlobalBaseline = captureGlobalBaseline();
	activeInlineWorkers++;
}

function releaseInlineGlobalBaseline(): void {
	if (activeInlineWorkers === 0) return;
	activeInlineWorkers--;
	if (activeInlineWorkers !== 0) return;
	const baseline = inlineGlobalBaseline;
	inlineGlobalBaseline = undefined;
	if (!baseline) return;
	...
	for (const key of Reflect.ownKeys(globalThis)) {
		...
		try {
			...
			// Best-effort cleanup for the inline fallback; keep reset/dispose non-throwing.
		}
	}
...
function spawnInlineWorker(): WorkerHandle {
	retainInlineGlobalBaseline();
	let released = false;
	const release = (): void => {
		if (released) return;
		released = true;
		releaseInlineGlobalBaseline();
	};
