...
export async function runEvalCompletion(
	...
	// when it is missing. Fall back to the static eval prompt so `completion(prompt)`
	// works without forcing every caller to pass a `system` prompt.
	const systemPrompt = system ? [system] : [evalLlmDefaultSystem.trim()];
	...
	try {
		response = await withBridgeTimeoutPause(options.emitStatus, () =>
		...
		throw new ToolError(formatProviderFailure("completion() request failed", classifyProviderFailure(error)));
	}
	...
	if (response.stopReason === "error") {
		throw new ToolError(
			formatProviderFailure(
				...
				classifyProviderFailure(response.errorMessage ?? "completion() request failed."),
			),
		);
	}
	if (response.stopReason === "aborted") {
		throw new ToolError(formatProviderFailure("completion() request aborted", classifyProviderFailure("aborted")));
	}
