...
export class VibeSessionRegistry {
	...
	#finishTurn(
		...
		record.lastActivityAt = Date.now();
		if (record.killed || resumable === false) {
			record.state = "dead";
			record.queue.length = 0;
			return;
		}
	...
	#settleTurn(
		...
		this.#finishTurn(session, manager, record, settledJobId, termination.resumable);
		const status = termination.status;
		const failed = status === "failed" || status === "aborted";
		const incomplete = status !== "completed";
		const reason = termination.reason || result.error || result.stderr || "";
		record.lastActivity = firstLine(
			incomplete ? `turn ${turnIndex} ${status}: ${reason}` : (result.lastIntent ?? result.output),
		);
		...
		try {
			text = prompt
				.render(vibeTurnResultTemplate, {
					...
					alive: termination.resumable && record.state !== "dead",
				})
				.trim();
		} catch (error) {
