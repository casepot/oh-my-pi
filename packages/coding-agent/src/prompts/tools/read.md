...
All `path` URI schemes resolve transparently and take the same line selectors. `artifact://<id>` recovers spilled output; large artifacts block unbounded `:raw`, so page with `artifact://<id>:N-M` / `artifact://<id>:raw:N-M` and use the reported artifact file path for search/copy workflows. `history://<agentId>` is an agent's transcript as concise markdown; bare `history://` lists agents.
