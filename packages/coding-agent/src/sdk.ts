...
function logMCPLoadErrors(errors: MCPToolsLoadResult["errors"] | MCPLoadResult["errors"]): void {
	const entries =
		errors instanceof Map ? [...errors].map(([path, error]) => ({ path: `mcp:${path}`, error })) : errors;
	for (const entry of entries) {
		logger.error("MCP tool load failed", entry);
	}
}
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const inheritedPromptCacheKey = sessionManager.getHeader()?.providerPromptCacheKey;
	if (forkCacheShapeChanged && inheritedPromptCacheKey !== undefined) {
		sessionManager.invalidateProviderPromptCacheKey(inheritedPromptCacheKey);
	}
	const providerPromptCacheKey =
		options.providerPromptCacheKey ?? (forkCacheShapeChanged ? undefined : inheritedPromptCacheKey);
	...
	const allRules = [...rulebookRules, ...alwaysApplyRules, ...ttsrManager.getRules()];
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			requestGoalTargetPlanApproval: (input, signal) => {
				if (!session) throw new Error("Goal mode is not active.");
				return session.requestGoalTargetPlanApproval(input, signal);
			},
			requestGoalTargetPlanFailure: (input, signal) => {
				if (!session) throw new Error("Goal mode is not active.");
		...
		if (settings.get("speechgen.enabled") && (!strictToolNames || strictToolNameSet.has(ttsTool.name))) {
		...
		if (customToolsLoadResult.tools.length > 0) {
			const loadedTools = strictToolNames
				? customToolsLoadResult.tools.filter(loaded => strictToolNameSet.has(loaded.tool.name.toLowerCase()))
				: customToolsLoadResult.tools;
			customTools.push(...loadedTools.map(loaded => loaded.tool));
		}
		...
		// Hard-isolated read-only advisor toolset, built unconditionally so advisor
		// configs can still select among the investigative defaults. Bind it to a
		// DISTINCT ToolSession (own `-advisor` session id + agent id) with fresh
		// read/write state so advisor reads, conflict registrations, and snapshots
		// never contaminate the primary session.
		...
		if (mcpManager && !options.mcpManager) {
			mcpManager.setOnToolsChanged(tools => {
				void (async () => {
					try {
						const discoveryWasEnabled = mcpDiscoveryEnabled;
						let activateAll =
							deferMCPDiscoveryForUI &&
							!discoveryWasEnabled &&
							!strictToolNames &&
							explicitlyRequestedMCPToolNames.length === 0;
						if (!discoveryWasEnabled && (await enableDeferredMCPDiscoveryForTools(session, tools))) {
							activateAll = false;
						}
						...
						if (deferMCPDiscoveryForUI && explicitlyRequestedMCPToolNames.length > 0) {
							if (mcpDiscoveryEnabled && !discoveryWasEnabled) {
		...
		startDeferredMCPDiscovery?.(session, {
			mcpDiscoveryEnabled,
			explicitlyRequestedMCPToolNames,
			activateAllMCPTools: !mcpDiscoveryEnabled && !strictToolNames && explicitlyRequestedMCPToolNames.length === 0,
		});
