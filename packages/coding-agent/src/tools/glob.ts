		...
		return untilAborted(signal, async () => {
			...
			for (const rawPattern of aliasResolvedPatterns) {
				...
				if (isSshUrl(rawPattern)) {
					throw new ToolError(
						`glob cannot operate on a remote ssh:// path: ${rawPattern}. ssh:// has no local file to glob; use \`read ${rawPattern}\` to list or inspect the remote path.`,
					);
				}
			...
			const buildResult = (
				...
				if (files.length === 0) {
					...
					if (limitClamped) {
						parts.push(
							`Glob limit ${limitClamped.requested} was clamped to ${limitClamped.effective}; narrow the glob before concluding absence beyond the shown window.`,
						);
					}
