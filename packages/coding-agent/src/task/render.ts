...
function renderAgentResult(
	...
	let statusLine = `${prefix ? `${prefix} ` : ""}${theme.fg(iconColor, icon)} ${theme.fg(
		...
		titlePart,
	)}${agentTypeBadge(result.agent, theme)} ${formatBadge(termination.status, statusColor, theme)}`;
	if (mergeFailed) {
...
export function renderResult(
	...
	const resultIds = new Set(details.results.map(result => result.id));
	const agentIds = new Set(resultIds);
	// Aggregate terminal state across finalized inline results and progress-only
	// siblings from mixed blocking/background calls. A result wins when an id is
	...
	for (const result of details.results) {
		requestTotal += result.requests ?? 0;
		switch (result.termination.status) {
			...
			case "paused":
				...
				break;
			case "completed":
				if (result.error) mergeFailedCount++;
				else successCount++;
				break;
		}
	}
	for (const progress of details.progress ?? []) {
		if (agentIds.has(progress.id)) continue;
		agentIds.add(progress.id);
		requestTotal += progress.requests ?? 0;
	...
	return framedBlock(theme, width => {
		...
		if (shouldRenderProgress && details.progress) {
			...
			const supplementalProgress = details.progress
				? orderProgressForDisplay(details.progress.filter(progress => !resultIds.has(progress.id)))
				: [];
