...
describe("task spawn policy surfaces", () => {
	...
	it("renders the restricted spawn default in the task description", async () => {
		...
		expect(description).toContain("Current spawn policy allows only `fact-finder`, `oracle`.");
		expect(description).toContain("Omitting `agent` selects `fact-finder`");
		expect(description).not.toContain("general-purpose worker (`fact-finder`)");
