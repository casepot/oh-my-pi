...
function statusGlyph(status: AgentStatus): string {
	switch (status) {
		...
		case "waiting":
			return theme.fg("accent", theme.status.pending);
		case "paused":
			return theme.fg("warning", theme.icon.pause || theme.status.pending);
		case "idle":
...
export class AgentHubOverlayComponent extends Container {
	...
	#renderTable(width: number): string[] {
		...
		if (this.#rows.length === 0) {
			...
			// its entries plus any top/bottom overflow indicators fill the budget,
			// so the selected entry stays centered without exceeding terminal rows.
			...
			for (let grew = true; grew; ) {
				...
				if (
					...
					used + entries[end].length + (start > 0 ? 1 : 0) + (end + 1 < entries.length ? 1 : 0) <= budget
				) {
				...
				if (
					start > 0 &&
					used + entries[start - 1].length + (start - 1 > 0 ? 1 : 0) + (end < entries.length ? 1 : 0) <= budget
				) {
					start--;
	...
	#renderEntry(ref: AgentRef, selected: boolean, width: number): string[] {
		...
		if (ref.statusDetail?.reason) {
			fields.push(theme.fg("muted", sanitizeLine(ref.statusDetail.reason, TRUNCATE_LENGTHS.TITLE)));
		}
