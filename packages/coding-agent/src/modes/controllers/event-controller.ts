...
/**
 * Loader label shown immediately after a user interrupt request, kept until
 * the active turn unwinds.
 */
...
export class EventController {
	...
	#updateWorkingMessageFromIntent(intent: unknown): void {
		if (this.#interrupting || this.ctx.session.isAborting) return;
		// Streamed JSON can deliver non-string intent values (object, number, boolean) before
	...
	#scheduleIdleRecap(): void {
		...
		if (!settings.get("recap.enabled")) return;
		if (this.ctx.editor.getText().trim()) return;

		const idleSeconds = settings.get("recap.idleSeconds");
		const timeoutMs = Math.max(IDLE_RECAP_MIN_SECONDS, Math.min(IDLE_RECAP_MAX_SECONDS, idleSeconds)) * 1000;
