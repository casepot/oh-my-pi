...
export class InteractiveMode implements InteractiveModeContext {
	...
	constructor(
		...
		this.goalTargetPanelContainer = new AnchoredLiveContainer();
	...
	#canConsiderGoalContinuation(): boolean {
		...
		if ((this.editor.pendingImages?.length ?? 0) > 0) return false;
	...
	async #enterPlanMode(options?: { planFilePath?: string; workflow?: "parallel" | "iterative" }): Promise<void> {
		...
		for (const toolName of ["eval", "task", "job", "irc"]) {
			if (this.session.getToolByName(toolName) !== undefined) {
				planAugmentations.push(toolName);
			}
		}
	...
	showNewVersionNotification(notification: string | StartupUpdateNotification): void {
		this.#uiHelpers.showNewVersionNotification(notification);
	}
