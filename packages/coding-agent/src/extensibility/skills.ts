...
export async function loadSkills(options: LoadSkillsOptions = {}): Promise<LoadSkillsResult> {
	...
	const includeUserSources =
		includeAllUserSources || enableCodexUser || enableClaudeUser || enablePiUser || enableAgentsUser;
	...
	const result = await loadCapability<CapabilitySkill>(skillCapability.id, {
		...
		disabledExtensions,
		includeUserSources,
		includeUserSourceProviders: [MANAGED_SKILLS_PROVIDER_ID],
	});
