...
function appendMarkdownTranscript(
	lines: string[],
	messages: readonly AgentMessage[],
	options: Pick<FormatSessionDumpTextOptions, "includeContextExcludedMessages" | "omitCustomTypes"> = {},
): void {
	for (const msg of messages) {
		if (msg.role === "user" || msg.role === "developer") {
			...
			if (options.includeContextExcludedMessages || !bashMsg.excludeFromContext) {
			...
			if (options.includeContextExcludedMessages || !pythonMsg.excludeFromContext) {
			...
			if (options.omitCustomTypes?.includes(customMsg.customType)) continue;
...
export function formatSessionDumpText(options: FormatSessionDumpTextOptions): string {
	...
	appendMarkdownTranscript(lines, options.messages, options);
	return lines.join("\n").trim();
}
