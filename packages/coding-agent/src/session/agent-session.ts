...
export class AgentSession {
	...
	#extractTtsrFilePathsFromArgs(args: unknown): string[] | undefined {
		return extractTtsrFilePathsFromToolArgs(args, { cwd: this.sessionManager.getCwd() });
	}
		...
		try {
			...
			if (contextWindow > 0) {
				if (typeof triggerContextTokens === "number" && Number.isFinite(triggerContextTokens)) {
					...
					const postShakeTokens = this.#estimateContextTokens().tokens;
	...
	#isTransientTransportErrorMessage(errorMessage: string): boolean {
		...
		return (
			...
			/overloaded|provider.?returned.?error|rate.?limit|too many requests|429|500|502|503|504|service.?unavailable|server.?error|internal.?error|retry your request|network.?error|connection.?error|connection.?refused|other side closed|fetch failed|upstream.?connect|upstream.?request.?failed|reset before headers|socket hang up|timed? out|timeout|terminated|retry delay|stream stall|typo in the url or port|no error details in response|HTTP2(?:StreamReset|RefusedStream|EnhanceYourCalm)/i.test(
				errorMessage,
			)
		);
	}
