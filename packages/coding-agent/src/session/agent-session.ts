...
const PLAN_DECISION_TOOLS: Record<string, true> = {
	ask: true,
	resolve: true,
};
...
export class AgentSession {
	...
	async #advanceReasoningSlide(liveMessages: AgentMessage[], context: AgentTurnEndContext | undefined): Promise<void> {
		...
		if (reasoningSlide.onFirstAction) {
			// Action trigger: the first successful edit/write marks the start of
			// the execution phase. Failed tool calls leave the slide armed.
			const action = context.toolResults.find(
				result => result.isError !== true && REASONING_SLIDE_ACTION_TOOLS[result.toolName],
			);
			if (!action) {
	...
	async #maintainAdvisorContext(advisor: ActiveAdvisor, incomingTokens: number): Promise<boolean> {
		...
		// Run compaction on advisor messages
		...
		for (const candidate of candidates) {
			...
			try {
				compactResult = await compact(
					...
					undefined,
					{
						thinkingLevel: candidate.thinkingLevel ?? advisorCompactionThinkingLevel ?? this.thinkingLevel,
	...
	buildTranscriptSessionContext(
		options?: Pick<BuildSessionContextOptions, "collapseCompactedHistory" | "keepDanglingToolCallIds">,
	): SessionContext {
		return deobfuscateSessionContext(
			this.sessionManager.buildSessionContext({
				...
				keepDanglingToolCallIds: options?.keepDanglingToolCallIds,
			}),
			this.#obfuscator,
		);
	}
	...
	async #writeGoalTranscriptFile(
		...
		const goal = this.#goalModeState?.goal;
		const goalHeader = goal ? `## Active Goal\n\nObjective: ${goal.objective}\nStatus: ${goal.status}\n\n` : "";
		const transcript = this.formatSessionAsText({
			includeContextExcludedMessages: true,
			omitCustomTypes: options.includeRubric ? undefined : [GOAL_RUBRIC_MESSAGE_TYPE],
		});
		await Bun.write(filePath, `${goalHeader}${transcript}`);
		return filePath;
	}
	...
	async sendCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "nextTurn") {
			...
			this.sessionManager.appendCustomMessageEntry(
				...
				normalizedAppMessage.includeInContext ?? true,
			);
			return false;
		}
	 ...
	 *
	 * Persists the prune and provider-usage floor reset in one atomic journal
	 * rewrite. The live agent context is rebuilt only after that transaction
	 * succeeds, so failed persistence cannot expose a partially-pruned branch.
	 */
	async #pruneStaleToolResults(): Promise<{ prunedCount: number; tokensSaved: number } | undefined> {
		...
		const result = await this.sessionManager.mutateEntriesAtomicallyIf(
			() => previewSupersededToolResultPrune(branchEntries, config).prunedCount > 0,
			() => {
				const pruned = pruneSupersededToolResults(this.sessionManager.getBranch(), config);
		...
		if (!result || result.prunedCount === 0) return undefined;
	...
	async handoff(customInstructions?: string, options?: SessionHandoffOptions): Promise<HandoffResult | undefined> {
		...
		try {
			...
			const handoffPromptText = renderHandoffPrompt(this.#obfuscateTextForProvider(effectiveCustomInstructions));
	...
	#estimatePrePromptContextTokens(messages: AgentMessage[], contextWindow: number): number {
		...
		const providerUsageFloor = this.model ? this.#estimateProviderUsageFloor(this.model, messages) : 0;
		// Floor by both the local estimate and the last same-model provider usage:
		// payload-shrinking hooks must not suppress pre-prompt compaction, and a
		// tiny pending prompt after a high-usage turn must still observe the
		// provider-billed context floor used by provider-call maintenance.
		return compactionContextTokens(
			compactionContextTokens(breakdown?.usedTokens ?? 0, localEstimate),
			providerUsageFloor,
		);
	}
	...
	#getCompactionModelCandidates(
		...
		return this.#resolveCompactionModelCandidates(this.model, availableModels, undefined, filter, true);
	}
	 ...
	 */
	async #runnableCompactionCandidates(
		candidates: readonly CompactionModelCandidate[],
		sessionId: string | undefined,
	): Promise<Model[]> {
		const keys = await Promise.all(
			candidates.map(candidate => this.#modelRegistry.getApiKey(candidate.model, sessionId)),
		);
		return candidates.filter((_, index) => keys[index] !== undefined).map(candidate => candidate.model);
	}
	...
	#resolveCompactionModelCandidates(
		...
		filter?: (model: Model) => boolean,
		includeModelConfiguredTarget = true,
	): CompactionModelCandidate[] {
		...
		if (includeModelConfiguredTarget && preferredModel) {
		...
		for (const role of MODEL_ROLE_IDS) {
			...
			addCandidate(resolved.model, concreteThinkingLevel(resolved.thinkingLevel));
		}
	...
	async #markCompactionUsageLimitReached(model: Model, message: string, signal?: AbortSignal): Promise<boolean> {
		if (!matchesUsageLimitText(message)) return false;
		const retryAfterMs =
	...
	async #compactWithFallbackModel(
		...
		precomputedCandidates?: CompactionModelCandidate[],
	...
	async #waitForActiveAutoCompaction(run: ActiveAutoCompactionRun): Promise<void> {
		try {
			...
			// Active auto-compaction handles and emits its own failure state.
		}
	}
	...
	async #runAutoCompaction(
		...
		if (activeRun) {
			const mustSupersedeForScheduling =
				requiresSchedulingSuppressed && activeRun.schedulesContinuation && priority >= activeRun.priority;
	...
	async #handleRetryableError(
		...
		if (!staleOpenAIResponsesReplayError && (!switchedCredential || retryBudgetExhausted) && currentSelector) {
		...
		if (retryBudgetExhausted) {
			...
			// The fallback model gets a fresh retry budget and lifecycle attempt —
			// leaving either counter spent would exhaust or mislabel its first retry.
			this.#retryAttempt = 1;
			attempt = 1;
		}
	...
	#estimateContextTokens(): {
		...
		const anchorMessage = messages[lastUsageIndex] as AssistantMessage;
		const providerNonMessage = anchorMessage.contextSnapshot?.nonMessageTokens;
