...
const customContinuationEntries = [
...
describe("buildSessionContext dangling toolCalls", () => {
	...
	it("keeps only explicitly executing dangling toolCalls in transcript mode", () => {
		const context = buildSessionContext(danglingToolCallEntries, undefined, undefined, {
			transcript: true,
			keepDanglingToolCallIds: new Set(["call-1"]),
		});
	...
	it("does not revive a historical dangling call during a custom continuation", () => {
		const context = buildSessionContext(customContinuationEntries, undefined, undefined, {
			transcript: true,
			keepDanglingToolCallIds: new Set(["call-current"]),
		});
