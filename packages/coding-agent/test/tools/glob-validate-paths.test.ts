...
import { GlobTool, globToolRenderer } from "../../src/tools/glob";
...
describe("glob diagnostics", () => {
	let tempDir: string;

	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "glob-diagnostics-"));
	...
	it("surfaces clamped limits in model text and structured details", async () => {
		...
		const result = await tool.execute("glob-clamped-limit", {
			paths: ["**/*.txt"],
			limit: 999,
		});
		...
		expect(text).toContain("Glob limit 999 was clamped to 200");
