...
describe("IRC", () => {
	...
	describe("IrcBus", () => {
		...
		it("wait with liveness survives a sender waiting transition and receives its later reply", async () => {
			const main = makeFakeSession();
			registry.register({
				id: "0-Main",
				displayName: "main",
			...
			registry.setStatus("0-Sub", "waiting", {
				code: "provider_retry",
				reason: "Retrying provider",
				since: Date.now(),
			});
			registry.setStatus("0-Sub", "running");
			await bus.send({ from: "0-Sub", to: "0-Main", body: "reply after retry" });

			await expect(waiting).resolves.toMatchObject({ from: "0-Sub", body: "reply after retry" });
		});
