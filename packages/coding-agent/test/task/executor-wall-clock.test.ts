...
describe("runSubprocess wall clock (task.maxRuntimeMs)", () => {
	...
	it("commits a valid terminal yield while the soft request budget is advisory", async () => {
	...
	it("does not finalize rejected yield arguments after crossing the advisory request budget", async () => {
		const settings = Settings.isolated({
			"task.softRequestBudget": 1,
			"task.softRequestBudgetNotice": true,
		});
	...
	it("resumes the hard runtime guard after an incremental yield commits", async () => {
		const settings = Settings.isolated({
			"task.softRequestBudget": 1,
			"task.softRequestBudgetNotice": true,
			"task.maxRuntimeMs": 30,
		});
		const firstAssistantMessage = {
			role: "assistant" as const,
		...
		let followingTurnObserved = false;
		const { promise: hang } = Promise.withResolvers<void>();
		const session: Partial<AgentSession> = {
			state: { messages: [] } as never,
			agent: { state: { systemPrompt: ["test"] } } as never,
			extensionRunner: undefined as never,
			...
			waitForIdle: async () => {
				...
				followingTurnObserved = true;
			},
			getLastAssistantMessage: () => lastAssistantMessage as never,
			abort: async () => {
				abortCount += 1;
				if (followingTurnObserved) {
