...
describe("task per-item blocking split", () => {
	...
	it("renders mixed result and progress ids in the aggregate status", () => {
		const inlineResult = makeResult("ScoutInline", "scout");
		...
		expect(lines[0]).toContain(theme.status.error);
		expect(text).toContain("WorkerFailed");
