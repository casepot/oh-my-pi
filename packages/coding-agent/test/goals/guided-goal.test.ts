...
describe("guided goal setup", () => {
	...
	it("salvages the latest guided objective when the turn cap ends on a question without one", async () => {
		...
		try {
			...
			vi.spyOn(harness.session, "createGoalWithRubric").mockImplementation(async input => {
				return await harness.session.goalRuntime.createGoal({ objective: input.objective });
			});
