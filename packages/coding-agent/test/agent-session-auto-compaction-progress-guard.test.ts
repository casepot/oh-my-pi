...
describe("AgentSession auto-compaction progress guard", () => {
	...
	it("keeps the visible overflow error when no recovery path is available", async () => {
		// When compaction is disabled there is no retry to run; the persisted
		// assistant error MUST stay on the branch so the user (and the reloaded
		// transcript) keeps the only explanation of why the turn stopped.
