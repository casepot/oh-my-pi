...
describe("provider prompt-cache key session affinity", () => {
	...
	it("initializes a same-project full fork with child request lineage and parent prompt-cache affinity", async () => {
	...
	it("does not inherit provider prompt-cache affinity across project directories", async () => {
		using tempDir = TempDir.createSync("@omp-prompt-cache-cross-project-");
		const source = await createSourceSessionFixture(tempDir, "parent-cache-session-cross-project");
		const targetCwd = tempDir.join("other-project");
		const targetSessionDir = tempDir.join("other-project-sessions");
		await fs.mkdir(targetCwd, { recursive: true });
		await fs.mkdir(targetSessionDir, { recursive: true });
		const forkedManager = await SessionManager.forkFrom(source.sourceFile, targetCwd, targetSessionDir);
		let session: AgentSession | undefined;
		let authStorage: AuthStorage | undefined;
		try {
			const created = await createMinimalSession(tempDir, {
			...
			expect(forkedManager.getHeader()?.parentSession).toBe(source.sourceHeader.id);
			expect(forkedManager.getHeader()?.providerPromptCacheKey).toBeUndefined();
	...
	it("durably invalidates changed-route affinity before resume and later forks", async () => {
		using tempDir = TempDir.createSync("@omp-prompt-cache-durable-invalidation-");
		const source = await createSourceSessionFixture(tempDir, "parent-cache-session-route-change");
		const forkedManager = await SessionManager.forkFrom(source.sourceFile, source.cwd, source.forkSessionDir);
		const forkedSessionId = forkedManager.getSessionId();
		...
		const resumedManager = await SessionManager.open(forkedSessionFile!, source.forkSessionDir);
		let resumedSession: AgentSession | undefined;
		let resumedAuthStorage: AuthStorage | undefined;
		try {
			const created = await createMinimalSession(tempDir, {
				cwd: source.cwd,
				sessionManager: resumedManager,
			});
			resumedSession = created.session;
			resumedAuthStorage = created.authStorage;

			expect(resumedManager.getHeader()?.providerPromptCacheKey).toBeUndefined();
			expect(resumedSession.agent.promptCacheKey).toBeUndefined();
		} finally {
			await resumedSession?.dispose();
			resumedAuthStorage?.close();
		}

		const reforkSessionDir = tempDir.join("reforked-sessions");
		await fs.mkdir(reforkSessionDir, { recursive: true });
		const reforkedManager = await SessionManager.forkFrom(forkedSessionFile!, source.cwd, reforkSessionDir);
		try {
			expect(reforkedManager.getHeader()?.parentSession).toBe(forkedSessionId);
			expect(reforkedManager.getHeader()?.providerPromptCacheKey).toBe(forkedSessionId);
			expect(reforkedManager.getHeader()?.providerPromptCacheKey).not.toBe(source.sourceHeader.id);
		} finally {
			await reforkedManager.close();
		}
	});
