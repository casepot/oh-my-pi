...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// Regression for issue #2401: a user who enables the canonical OMP-native
		// `~/.agent[s]/skills` source MUST still see those skills when the named
		// third-party CLI toggles (codex/claude/native) are disabled. The fork keeps
		// user/home discovery opt-in, so this test enables only `agents:user`.
		it("should load ~/.agents/skills when agents:user is enabled and third-party toggles are off (#2401)", async () => {
