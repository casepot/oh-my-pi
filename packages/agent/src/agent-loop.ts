...
const NO_COMPLETED_TOOL_CALLS: ReadonlySet<string> = new Set();
...
async function finalizeProviderContextForDispatch(
	providerContext: Context,
	config: AgentLoopConfig,
	ownedDialect: Dialect | undefined,
): Promise<FinalizedProviderContext> {
	let finalContext = providerContext;
	if (config.transformProviderContext) {
		finalContext = await config.transformProviderContext(finalContext, config.model);
	}
		...
		while (true) {
			...
			while (hasMoreToolCalls || pendingMessages.length > 0) {
				...
				if (toolCalls.length > 0) {
					...
					if (message.stopReason !== "length" || deadlinePassed) {
...
async function streamAssistantResponse(
	...
	const initialProviderContext = await materializeProviderContext(
		context,
		config,
		signal,
		exampleDialect,
	...
	let finalized = await finalizeProviderContextForDispatch(initialProviderContext, config, ownedDialect);
	for (let rematerializations = 0; config.preflightProviderContext; ) {
		...
		rematerializations++;
		const rematerializedProviderContext = await materializeProviderContext(
			context,
			config,
			signal,
			exampleDialect,
		...
		finalized = await finalizeProviderContextForDispatch(rematerializedProviderContext, config, ownedDialect);
	}
