...
describe("agentLoopContinue with AgentMessage", () => {
	...
	it("preserves an external abort boundary when a completed tool ignores cancellation", async () => {
		...
		const config: AgentLoopConfig = {
			model: mock.model,
			convertToLlm: identityConverter,
			preflightProviderContext: async input => {
	...
	it("stops promptly when a running tool cooperates with external cancellation", async () => {
		const toolSchema = type({ value: "string" });
		const controller = new AbortController();
		const started = Promise.withResolvers<void>();
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
		...
		const mock = createMockModel({
			responses: [
				{ content: [{ type: "toolCall", id: "tool-cancelled", name: "echo", arguments: { value: "stop" } }] },
				{ content: ["must not be reached"] },
			],
		});
		const config: AgentLoopConfig = {
