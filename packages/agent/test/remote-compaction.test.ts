...
describe("requestOpenAiRemoteCompaction timeout", () => {
	test("a never-responding endpoint rejects with structured timeout details instead of hanging", async () => {
		...
		await expect(
			...
			details: { kind: "timeout", timedOut: true, timeoutMs: 20 },
		});
	});
});
...
describe("compact() remote compaction failure handling", () => {
	...
	test("re-expands a prior V2 compaction's originals when no candidate can reuse the replay", async () => {
		...
		const reused = prepareCompaction(
			entries,
			{ ...baseSettings, remoteEnabled: true, remoteStreamingV2Enabled: true },
			[v2Model],
		);
