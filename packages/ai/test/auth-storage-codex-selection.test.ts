...
describe("AuthStorage codex oauth ranking", () => {
	...
	test("skips definitively failed preflight refreshes and selects refreshable sibling", async () => {
		if (!authStorage) throw new Error("test setup failed");
		...
		const apiKey = await authStorage.getApiKey("openai-codex", "refresh-failure-selection");
