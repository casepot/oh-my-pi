...
function countOpenAIResponsesVisibleTextChars(output: AssistantMessage): number {
	let chars = 0;
...
function applyOpenAIResponsesIncompleteStopDetails(
	output: AssistantMessage,
	response:
		| {
				status?: string;
	...
	const stopDetails: NonNullable<AssistantMessage["stopDetails"]> = {
		...
		visibleTextChars: countOpenAIResponsesVisibleTextChars(output),
	};
