...
function countCodexVisibleTextChars(output: AssistantMessage): number {
	let chars = 0;
...
function isIncompleteCodexToolCallItem(item: Record<string, unknown>): boolean {
	const itemType = item.type;
	return (itemType === "function_call" || itemType === "custom_tool_call") && item.status === "incomplete";
}

function encodeIncompleteCodexToolCallId(item: Record<string, unknown>): string | undefined {
	const callId = item.call_id;
	if (typeof callId !== "string" || callId.length === 0) return undefined;
	const itemId = typeof item.id === "string" ? item.id : undefined;
	return encodeResponsesToolCallId(callId, itemId);
}

function dropIncompleteCodexToolCalls(output: AssistantMessage, runtime: CodexStreamRuntime): void {
	const incompleteToolCallIds = new Set<string>();
	const retainedNativeItems: Array<Record<string, unknown>> = [];
	for (const item of runtime.nativeOutputItems) {
		if (isIncompleteCodexToolCallItem(item)) {
			const toolCallId = encodeIncompleteCodexToolCallId(item);
			if (toolCallId) incompleteToolCallIds.add(toolCallId);
			continue;
		}
		retainedNativeItems.push(item);
	}
	if (retainedNativeItems.length === runtime.nativeOutputItems.length) {
		return;
	}
	runtime.nativeOutputItems.splice(0, runtime.nativeOutputItems.length, ...retainedNativeItems);
	if (incompleteToolCallIds.size === 0) {
		return;
	}
	output.content = output.content.filter(block => block.type !== "toolCall" || !incompleteToolCallIds.has(block.id));
}
...
function createRequestSetup(options: OpenAICodexResponsesOptions | undefined): CodexRequestSetup {
	...
	const wrapCodexSseStream = (
		...
		iterateWithIdleTimeout(source, {
			...
			firstItemErrorMessage: CODEX_SSE_FIRST_EVENT_TIMEOUT_MESSAGE,
...
class CodexStreamProcessor {
	...
	#handleResponseCompleted(rawEvent: Record<string, unknown>): void {
		...
		const incompleteReason =
			response && "incomplete_details" in response
				? parseCodexIncompleteDetailsReason(response.incomplete_details)
				: undefined;
		const responseIncomplete = status === "incomplete";
		...
		if (responseIncomplete) {
			dropIncompleteCodexToolCalls(output, runtime);
		}
		...
		if (responseIncomplete) {
			const stopDetails: NonNullable<AssistantMessage["stopDetails"]> = {
				type: incompleteReason || "incomplete",
				category: "response.incomplete",
				status: status ?? "incomplete",
				visibleTextChars: countCodexVisibleTextChars(output),
			};
			if (typeof usage?.output_tokens === "number") {
				stopDetails.outputTokens = usage.output_tokens;
			}
			const reasoningTokens = usage?.output_tokens_details?.reasoning_tokens;
			if (typeof reasoningTokens === "number" && reasoningTokens > 0) {
				stopDetails.reasoningTokens = reasoningTokens;
			}
			if (turnStateReset) {
				stopDetails.turnStateReset = true;
			}
	...
	finalize(completion: CodexStreamCompletion): AssistantMessage {
		...
		output.providerPayload = createOpenAIResponsesHistoryPayload(
			this.model.provider,
			filterCodexHistoryItemsForPersistence(this.runtime.nativeOutputItems),
		);
...
function parseCodexIncompleteDetailsReason(value: unknown): string | undefined {
	if (!value || typeof value !== "object" || !("reason" in value)) return undefined;
...
async function openCodexSseEventStream(
	...
	try {
		...
		if (watchdog.timedOut() && !signal?.aborted) {
			throw new AIError.StreamTimeoutError(CODEX_SSE_FIRST_EVENT_TIMEOUT_MESSAGE, { cause: error });
		}
		throw error;
	} finally {
		watchdog.clear();
		...
		if (msg.role === "assistant") {
			...
			if (historyItems) {
				const sanitizedHistoryItems = sanitizeOpenAIResponsesAssistantHistoryItemsForReplay(
					filterCodexHistoryItemsForPersistence(historyItems),
				);
...
const optionalCodexString = type("unknown").pipe((raw: unknown) => {
...
const codexErrorDetailSchema = type("unknown").pipe((raw: unknown) => {
...
const innerFailureEventSchema = type({
	...
	"response?": type("unknown").pipe((raw: unknown) => {
		const out = type({
			"error?": codexErrorDetailSchema,
			"message?": optionalCodexString,
			"status?": optionalCodexString,
...
const codexFailureEventSchema = type("unknown").pipe((raw: unknown) => {
