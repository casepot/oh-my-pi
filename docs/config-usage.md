...
- `Settings.init()` loads global `config.yml` + discovered project settings capability items (`settings.json`, then `config.yml` from providers).
...
- Settings key migrations include `queueMode`, `ask.timeout`, flat `theme`, `task.isolation.enabled`, legacy `task.isolation.mode` values, removed edit modes, `statusLine.plan_mode`, `compaction.strategy: "shake-summary" -> "shake"`, `memories.enabled`, legacy `memory.backend: "mnemosyne" -> "mnemopi"`, top-level `mnemosyne` settings -> `mnemopi`, and hindsight scoping/name fields.
