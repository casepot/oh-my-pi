...
8. active skillset-provided rules (merged later in SDK; normal discovery still wins by name)
...
6. If TTSR is enabled, register unforced rules with non-empty `condition` (regex) or `astCondition` (ast-grep patterns) into `TtsrManager`; accepted rules are TTSR-only.
7. If TTSR is disabled, suppress unforced condition/AST-condition rules instead of promoting them into the rulebook.
8. Put remaining `alwaysApply === true` rules into `alwaysApplyRules`.
9. Put remaining rules with `description` into `rulebookRules`.

### Bucket behavior

- **TTSR bucket**: any unforced enabled rule with a non-empty parsed `condition` (regex) or `astCondition` (ast-grep patterns) that `TtsrManager.addRule(...)` accepts while `ttsr.enabled !== false`. TTSR-only rules are not visible through `rule://`.
...
- A rule with both a trigger condition and `alwaysApply` goes to TTSR only if TTSR registration accepts it; otherwise it can fall through to always-apply when not suppressed by disabled TTSR semantics.
- A rule with both `alwaysApply` and `description` goes to always-apply only (not rulebook).
- A condition/AST-condition rule with invalid TTSR metadata may fall through to rulebook only when TTSR is enabled and the rule has `description`; `ttsr.enabled: false` suppresses unforced condition rules.
