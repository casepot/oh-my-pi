...
> Spawn subagents — one per call, or a `tasks[]` batch per call (`task.batch`, default on). With `async.enabled=true`, spawns run in the background; otherwise the call blocks until they finish. Execution mode is per item: an item whose resolved agent definition declares `blocking: true` runs inline and returns its result in the call, while non-blocking items in the same call still spawn as background jobs. The bundled `scout` does not declare `blocking`, so with `async.enabled=true` it runs as a background job.
...
- status: `exitCode`, optional `error`, `providerNotice`, and `retryFailure`, plus mandatory `termination: { status, code, reason, resumable, historyUri, outputUri, policy }`
...
- Agent source precedence: project custom agents, then user custom agents, then OMP extension-package agents, then Claude plugin agents (project scope before user scope), then bundled agents (`scout`, `designer`, `reviewer`, `task`, `sonic`, `librarian`).
...
- Agent discovery precedence is first-wins by exact name: project `.omp` agents dir, user `.omp` agents dir, OMP extension-package agent dirs in extension source order, Claude marketplace plugin agent dirs (project scope before user scope), then bundled agents. Direct `.claude`/`.codex`/`.gemini` agent directories are skipped. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
...
- Configured subagent models must have working credentials. `task.fallbackToParentModelOnAuthFailure` defaults to `false`; when explicitly enabled, an unavailable subagent model may fall back to the authenticated parent-session model and reports that substitution.
