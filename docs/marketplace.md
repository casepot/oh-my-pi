...
A **plugin** is a directory containing Claude/OMP plugin content such as skills, skillsets, commands, hooks, tools, MCP servers, LSP servers, rules, prompts, or extension modules. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).
