...
A **plugin** is a directory containing Claude/OMP plugin content such as skills, skillsets, commands, agents, hooks, tools, MCP servers, LSP servers, rules, prompts, or marketplace metadata. Extension modules (`package.json` `omp.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `omp plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).
...
Local marketplace sources are normalized to absolute paths after `~` expansion. Relative plugin sources are normalized inside the marketplace root (after `metadata.pluginRoot`), and plugin source paths that escape that root are rejected.
