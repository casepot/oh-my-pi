...
- passes `authStorage`, cache storage, `mcp.enableProjectConfig` / `mcp.enableUserConfig` settings, and browser-MCP filtering based on the `browser.enabled` setting,
...
- `enableUserConfig: false` removes user-level entries (`_source.level === "user"`).
